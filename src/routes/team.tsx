...
import { Plus, Pencil, Trash2, Crown, DollarSign, ClipboardCheck, Users, Gauge, Layers, TriangleAlert } from "lucide-react";
...
function TeamPage() {
  ...
  const loadOf = (m: TeamMember) => {
    const active = projects.filter((p) => p.ownerId === m.id && p.stage !== "Archived" && p.stage !== "Delivered").length;
    const up = shoots.filter((s) => s.crewIds.includes(m.id) && new Date(s.date) >= new Date(new Date().toDateString())).length;
    return Math.min(100, Math.round(((active * 10 + up * 8) / (m.capacity ?? 40)) * 100));
  };
  const loads = team.map(loadOf);
