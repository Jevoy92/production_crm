...
function MetaItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-lo text-xs">{label}:</span>
      <span className="text-hi text-xs font-semibold">{value}</span>
    </div>
  );
}
function MetaSep() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-5 flex-wrap mb-5 pb-5 border-b border-line">
        <MetaItem icon={<ExternalLink size={13} />} label="Client" value={client?.company ?? client?.name ?? "—"} />
        <MetaSep />
        <MetaItem icon={<span className="w-2 h-2 rounded-full" style={{ background: palColor(project.palType) }} />} label="Type" value={project.palType} />
        <MetaSep />
        <MetaItem icon={<Calendar size={13} />} label="Shoot" value={project.shootDate ? new Date(project.shootDate).toLocaleDateString(undefined, { month: "short", day: "numeric" }) : "TBD"} />
        <MetaSep />
        <MetaItem icon={<Calendar size={13} />} label="Delivery" value={project.deliveryDate ? new Date(project.deliveryDate).toLocaleDateString(undefined, { month: "short", day: "numeric" }) : "TBD"} />
        <MetaSep />
        <MetaItem icon={<DollarSign size={13} />} label="Budget" value={project.quoted ? `$${project.quoted.toLocaleString()}` : "—"} />
        <div className="ml-auto flex items-center gap-2.5">
          <span className="text-lo text-xs">Overall</span>
          <div className="w-28 h-1.5 bg-sunken rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-brand-500 to-cyan-500" style={{ width: `${prog.pct}%` }} />
          </div>
          <span className="text-hi text-xs font-semibold num">{prog.pct}%</span>
        </div>
      </div>
