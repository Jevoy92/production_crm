...
export function MonthGenerator({
  ...
  const run = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Generation failed. Check LOVABLE_API_KEY and try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <select
            ...
            value={ventureId}
            onChange={(e) => { setVentureId(e.target.value as VentureId); reset(); }}
          >
        ...
        <label className="block">
          ...
          <input
            type="number" min={1} max={40} className="ph-input w-full"
            value={postCount}
        ...
        <label className="block">
          ...
          <select className="ph-select w-full" value={month} onChange={(e) => setMonth(Number(e.target.value))}>
        ...
        <label className="block">
          ...
          <input
            type="number" min={2024} max={2100} className="ph-input w-full"
            value={year}
      ...
      <div className="mt-3 grid grid-cols-1 gap-3">
        <label className="block">
          <span className="text-xs text-lo mb-1 block">Focus areas this month <span className="text-lo/70">(optional, comma-separated)</span></span>
          <input
        ...
        <label className="block">
          <span className="text-xs text-lo mb-1 block">Extra context <span className="text-lo/70">(optional)</span></span>
          <textarea
            rows={2} className="ph-input w-full"
      ...
      <div className="mt-3 text-[11.5px] text-lo rounded-lg bg-sunken border border-line px-3 py-2">
        <span className="font-medium" style={{ color: venture.accent }}>{venture.shortName}</span> · {venture.tone}
      </div>

      {error && (
        <div className="mt-3 text-sm text-rose bg-rose/10 border border-rose/25 rounded-lg px-3 py-2">{error}</div>
      )}
      ...
      {posts && (
        <div className="mt-4">
          ...
          <div className="space-y-1.5 max-h-[44vh] overflow-y-auto pr-1">
            {[...posts].sort((a, b) => a.dayOfMonth - b.dayOfMonth).map((p, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl bg-sunken border border-line px-3 py-2.5">
                <div className="text-center w-9 flex-shrink-0">
                  <div className="text-[9px] uppercase tracking-wider text-lo">{MONTHS[month - 1].slice(0, 3)}</div>
                  <div className="num text-base font-bold leading-none text-hi">{p.dayOfMonth}</div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-hi leading-snug">{p.title}</div>
                  {p.hook && <div className="text-[12px] text-mid italic mt-0.5 line-clamp-1">“{p.hook}”</div>}
...
function Chip({ children, accent }: { children: React.ReactNode; accent?: string }) {
  return (
    <span
      ...
      style={
        accent
          ? { color: accent, background: `color-mix(in oklab, ${accent} 14%, transparent)`, borderColor: `color-mix(in oklab, ${accent} 30%, transparent)` }
          : undefined
      }
    >
      {children}
