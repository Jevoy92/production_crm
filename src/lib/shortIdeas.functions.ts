...
export const generateShortIdeas = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const system = [
      ...
      "The three must differ in format, not just wording: e.g. a live demo/experiment, a visual metaphor built on camera, a character/bit, a street or in-the-wild test, a time-lapse build, a silent-visual with text.",
    ...
    const prompt = [
      ...
      '{"ideas":[{"title":"short punchy concept name","prop":"the physical prop","premise":"what happens on camera, 1-2 sentences","hook":"first spoken line, under 7 seconds","beats":["beat 1","beat 2","beat 3","beat 4"],"tieBack":"how it connects back to the long-form idea","cta":"closing call to action","durationSec":45}]}',
    ].join("\n");
